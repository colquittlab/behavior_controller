...
if __name__ == "__main__":
	...
	if os.path.isfile(udev_rules_fpath):
		...
		if syspath in device_dict.keys():
			...
			iresult = input('Rename? ["y" or any other key to exit] ')
			if str(iresult).lower() != 'y':
				sys.exit()
	else:
		iresult = input('No udev file found at %s    Create file? ["y" or any key to exit] ' % udev_rules_fpath)
		if str(iresult).lower()=='y':
			device_dict = {}
		else:
			sys.exit()
	...
	while not portnumassigned:
		iresult = input('Enter audioport number for usb audio port %s  ' % syspath)
		try:
			...
			if port_number in device_dict.values():
				if syspath in device_dict.keys() and device_dict[syspath] == port_number:
					...
					iresult = input('port %d is already assigned. Enter "o" overwrite, "n" to enter a new port assignment or "q" to quit ')
